...
const TicketAction:FC<TicketActionProps> = ({assigned}) => {
  const {state} = useContext(MainContext);

  if (assigned && state?.user?.full_name !== assigned) return <p className="text-primary">Только для чтения</p>;

  return (
    <button className="rounded bg-primary px-4 py-2 text-white hover:bg-agree">{assigned ? 'Закрыть заявку' : 'Взять в работу'}</button>
  );
};
